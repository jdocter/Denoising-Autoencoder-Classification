...
class ASL:
    ...
    def cnn_setup(self):
        ...
        # Used for sample weight experiment
        # self.train_is_labeled = np.zeros((self.n_labeled + self.n_unlabeled))
        # self.train_is_labeled[:self.n_labeled] = 1.0
        # self.train_is_labeled[self.n_labeled:] = 1e-6
        # self.train_is_labeled = self.train_is_labeled[random_ordering]
    ...
    def create_cnn_model(self, regularized):
        ...
        decode = Dense(2048, activation='relu')(encode)
        decode = Reshape((16, 16, 8))(decode)
        # decode = Dropout(0.25)(decode)
        decode = UpSampling2D(size=(2, 2))(decode)
        decode = Conv2D(3, kernel_size=(3, 3), activation='sigmoid', padding='same', name='reconstruction', kernel_initializer=self.my_init)(decode)
    ...
    def create_simple_model(self, regularized): # add a Dense layer with a L1 activity regularizer
        ...
        decode = Dense(3072, activation='sigmoid',name='reconstruction')(encode)
    ...
    def train_regularized_model(self):
        ...
        # Experimented with hinge loss, didn't work

        model.compile(loss={'class' : self.conditional_categorical_crossentropy, 'reconstruction' : 'mse'},
        ...
        model.fit(np.copy(self.x_train_all_shuffled),
                  ...
                  # sample_weight={
                      # learn classification from labeled data, auto-encoding from unlabeled data
                      # 'class' : self.train_is_labeled,
                      # 'reconstruction' : 1.0 + 1e-6 - self.train_is_labeled},
